...
const BlogList = () => {
  ...
  useEffect(() => {
    const fetchCategoryItem = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/blogcategory`);
        const data = await response.json();
        setCategories(data);
      } catch (error) {
        console.error("Error fetching data", error);
                    ...
                    >
                      ...
                      <div className="w-full md:w-1/2">
                        ...
                        <div className="body3 text-secondary mt-4 pb-4">
                          {getTextFromHTML(item.long_description)}
                        </div>
          ...
          <div className="w-full lg:w-1/3 lg:pl-[55px]">
            <div className="search-block rounded-lg bg-surface h-[50px] relative">
              <input
                ...
                placeholder="Search"
              />
              <Icon.MagnifyingGlassIcon className="absolute top-1/2 -translate-y-1/2 right-4 text-xl cursor-pointer" />
            </div>
            <div className="cate-block md:mt-10 mt-6">
              ...
              <div className="list-nav mt-4">
                <div
                  className={`text-button text-secondary mt-2 cursor-pointer ${selectedCategory === null ? "font-extrabold" : ""}`}
                ...
                {categories.map((item, index) => (
                  <div
                    className={`text-button text-secondary mt-2 cursor-pointer ${selectedCategory === item.id ? "font-extrabold" : ""}`}
